...
def main (model_class_name=None,
    ...
    try:
        ...
        if not is_interactive:
            cfg.ask_settings(mode_name=mode_name,
                             mask_mode_name=mask_mode_name, 
                             erode_mask=erode_mask,
                             blur_mask=blur_mask)
            
        #subprocess_count = io.input_int("Number of workers?", max(8, multiprocessing.cpu_count()), valid_range=[1, multiprocessing.cpu_count()], help_message="Specify the number of threads to process. A low value may affect performance. A high value may result in memory error. The value may not be greater than CPU cores." )
        subprocess_count = 8
