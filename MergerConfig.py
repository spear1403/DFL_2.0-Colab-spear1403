...
class MergerConfigMasked(MergerConfig):
    ...
    def ask_settings(self,
        ...
        # self.mode = mode_dict.get (mode, self.default_mode )
        self.mode = mode_name
        io.log_info(f"Chosen mode is {self.mode}")
        ...
        # io.log_info(s)
        for key, value in mask_mode_dict.items():
            if value == mask_mode_name:
                self.mask_mode = key
                break
            else:
                self.mask_mode = 1
        io.log_info(f"Chosen mask mode is {value}")

        if 'raw' not in self.mode:
            self.erode_mask_modifier = np.clip ( erode_mask, -400, 400)
            io.log_info(f"Erode mask modifier set to {erode_mask}")
            self.blur_mask_modifier =  np.clip ( blur_mask, 0, 400)
            io.log_info(f"Blur mask modifier set to {blur_mask}")
            self.motion_blur_power = np.clip ( 0, 0, 100)

        self.output_face_scale = np.clip (0, -50, 50)

        if 'raw' not in self.mode:
            self.color_transfer_mode = "mkl"
